...
def accuracy(pred, targets):
    with torch.no_grad():
        accuracy = sum(pred.argmax(dim=1) == targets.argmax(dim=1)) / len(targets)
...
def f1(pred, targets):
    ...
    with torch.no_grad():
        ...
        f1 = metrics.f1_score(targets.cpu().numpy(), pred.cpu().numpy())
    return f1.item()


def expand_targets(targets):
    return torch.tensor(list(map(lambda x: [1, 0] if x ==0 else [0, 1], targets)))


def predict_single(x, y, model, timesteps, device):
    spk, _ = forward_pass(
        model, torch.tensor([x, y], dtype=torch.float32).to(device), timesteps
    )
...
def predict(data, model, timesteps, device):
    if data.get_device() == -1:
        spk, _ = forward_pass(
            model, torch.tensor(data, dtype=torch.float32).to(device), timesteps
        )
    else:
    ...
    for name, train_loader, test_loader in dataloaders:
        ...
        for seed in config.model_seeds:
            ...
            optimizer = torch.optim.Adam(
                net.parameters(), lr=config.learning_rate, betas=config.adam_betas
            )
            ...
            for epoch in tqdm(range(config.epochs)):
                ...
                for i, (data, targets) in enumerate(iter(train_loader)):
                    data = data.to(device)
                    targets = expand_targets(targets.squeeze()).type(torch.float32).to(device)
                    pred = net(data) # musze spasowac targets do postaci [x, y] zeby policzyc dystanse
                    net.train()
                    loss_val = loss_fn(pred, targets)
                    optimizer.zero_grad()  # null gradients
                    loss_val.backward()  # calculate gradients
                    optimizer.step()  # update weights
                    train_epoch_loss_val += loss_val.item()
                    train_epoch_acc_val += accuracy(pred, targets)
                    train_epoch_f1_val += f1(pred, targets)
                ...
                for i, (data, targets) in enumerate(iter(test_loader)):
                    ...
                    test_epoch_acc_val += accuracy(pred, targets)
                    test_epoch_f1_val += f1(pred, targets)
                ...
                update_results_df(
                    ...
                    epoch,
                    {
                        "loss": train_epoch_loss_val,
                        "accuracy": train_epoch_acc_val,
                        "f1": train_epoch_f1_val,
                    },
                    {
                        "loss": test_epoch_loss_val,
                        "accuracy": test_epoch_acc_val,
...
def dump_results(results) -> None:
    ...
    filename = f"results_{git_hash}_{snn_version}_ANN.hdf"
...
def main():
    ...
    dataloaders = (
        (
            "AND",
            DataLoader(continous_and_generator(size=700), config.batch_size),
            DataLoader(continous_and_generator(size=300), config.batch_size),
        ),
        (
            "OR",
            DataLoader(continous_or_generator(size=700), config.batch_size),
            DataLoader(continous_or_generator(size=300), config.batch_size),
        ),
        (
            "XOR",
            DataLoader(continous_xor_generator(size=700), config.batch_size),
            DataLoader(continous_xor_generator(size=300), config.batch_size),
        ),
    )
